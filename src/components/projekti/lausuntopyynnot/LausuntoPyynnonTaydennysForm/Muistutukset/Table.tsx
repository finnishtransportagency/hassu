...
export default function AineistoTable({
  ...
  const { fields, move, remove } = useFieldArray({ name: `lausuntoPyynnonTaydennykset.${lptIndex}.muistutukset`, control });
  const { append: appendToPoistetut } = useFieldArray({ name: `lausuntoPyynnonTaydennykset.${lptIndex}.poistetutMuistutukset`, control });
    ...
    () => [
      {
        ...
        accessorFn: (aineisto) => {
          ...
          const errorMessage = (formState.errors.lausuntoPyynnonTaydennykset?.[lptIndex]?.muistutukset?.[aineistoIndex] as any | undefined)
            ?.message;
          return (
            <>
              ...
              <input type="hidden" {...register(`lausuntoPyynnonTaydennykset.${lptIndex}.muistutukset.${aineistoIndex}.tiedosto`)} />
              <input type="hidden" {...register(`lausuntoPyynnonTaydennykset.${lptIndex}.muistutukset.${aineistoIndex}.nimi`)} />
            </>
          );
        },
      },
  ...
  const onDragAndDrop = useCallback(
    (id: string, targetRowIndex: number) => {
      ...
      setValue(`lausuntoPyynnonTaydennykset.${lptIndex}.muistutukset.${aineistoIndex}.jarjestys`, targetRowIndex);
      setValue(`lausuntoPyynnonTaydennykset.${lptIndex}.muistutukset.${targetRowIndex}.jarjestys`, aineistoIndex);
...
type ActionColumnProps = {
  ...
  appendToPoistetut: UseFieldArrayAppend<LausuntoPyynnonTaydennysFormValues, `lausuntoPyynnonTaydennykset.${number}.poistetutMuistutukset`>;
  index: number;
