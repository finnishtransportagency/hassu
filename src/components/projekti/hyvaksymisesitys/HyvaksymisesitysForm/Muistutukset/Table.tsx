...
export default function AineistoTable({
  lptIndex,
  joTallennetutMuistutukset,
}: {
  lptIndex: number;
  joTallennetutMuistutukset: LadattuTiedosto[];
}) {
  const { control, formState, register, setValue } = useFormContext<HyvaksymisesitysFormValues>();
  const { fields, move, remove } = useFieldArray({ name: `hyvaksymisesitykset.${lptIndex}.muistutukset`, control });
  const { append: appendToPoistetut } = useFieldArray({ name: `hyvaksymisesitykset.${lptIndex}.poistetutMuistutukset`, control });

  const enrichedFields: FormLadattuTiedosto[] = useMemo(
    () =>
      fields.map((field) => {
        const aineistoData = joTallennetutMuistutukset || [];
        const { tuotu } = aineistoData.find(({ uuid }) => uuid === field.uuid) || {};

        return { tuotu, ...field };
      }),
    [fields, joTallennetutMuistutukset]
  );
  ...
  const columns = useMemo<ColumnDef<FormLadattuTiedosto>[]>(
    () => [
      {
        ...
        accessorFn: (aineisto) => {
          ...
          const errorMessage = (formState.errors.hyvaksymisesitykset?.[lptIndex].muistutukset?.[aineistoIndex] as any | undefined)
            ?.message;
          return (
            <>
              ...
              <input type="hidden" {...register(`hyvaksymisesitykset.${lptIndex}.muistutukset.${aineistoIndex}.tiedosto`)} />
              <input type="hidden" {...register(`hyvaksymisesitykset.${lptIndex}.muistutukset.${aineistoIndex}.nimi`)} />
            </>
          );
        },
      },
    ...
    [enrichedFields, formState.errors.hyvaksymisesitykset, lptIndex, register, remove, appendToPoistetut]
  );
  ...
  const onDragAndDrop = useCallback(
    (id: string, targetRowIndex: number) => {
      ...
      setValue(`hyvaksymisesitykset.${lptIndex}.muistutukset.${aineistoIndex}.jarjestys`, targetRowIndex);
      setValue(`hyvaksymisesitykset.${lptIndex}.muistutukset.${targetRowIndex}.jarjestys`, aineistoIndex);
...
type ActionColumnProps = {
  ...
  appendToPoistetut: UseFieldArrayAppend<HyvaksymisesitysFormValues, `hyvaksymisesitykset.${number}.poistetutMuistutukset`>;
  index: number;
