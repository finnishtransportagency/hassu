...
export default function Lukunakyma() {
  ...
  const julkaisu = useMemo(() => projekti?.jatkoPaatos1VaiheJulkaisut?.[projekti.jatkoPaatos1VaiheJulkaisut.length - 1], [projekti]);

  const { published } = examineJulkaisuPaiva(julkaisu?.tila === HyvaksymisPaatosVaiheTila.HYVAKSYTTY, julkaisu?.kuulutusPaiva);
