...
import { HyvaksymisPaatosVaiheTila, Projekti, ProjektiKayttaja, YhteystietoInput, KayttajaTyyppi } from "@services/api";
...
export function hasHyvaksyttyJatkoPaatos1VaiheJulkaisu(projekti: Projekti | null | undefined) {
  return (
    (projekti?.jatkoPaatos1VaiheJulkaisut?.filter((julkaisu) => julkaisu.tila == HyvaksymisPaatosVaiheTila.HYVAKSYTTY) || []).length > 0
  );
}
...
export default function EsitettavatYhteystiedot({}: Props): ReactElement {
  ...
  const eiVoiMuokata = hasHyvaksyttyJatkoPaatos1VaiheJulkaisu(projekti);
  ...
  const { fields, append, remove } = useFieldArray({
    control,
    name: "jatkoPaatos1Vaihe.kuulutusYhteystiedot.yhteysTiedot",
  });

  const kuulutusYhteysHenkilot: ProjektiKayttaja[] = projekti?.jatkoPaatos1Vaihe?.kuulutusYhteystiedot?.yhteysHenkilot
    ...
    <Section>
      <SectionContent>
        ...
        {projekti?.kayttoOikeudet && projekti.kayttoOikeudet.length > 0 ? (
          <Controller
            control={control}
            name={`jatkoPaatos1Vaihe.kuulutusYhteystiedot.yhteysHenkilot`}
      ...
      {fields.map((field, index) => (
        <HassuStack key={field.id} direction={["column", "column", "row"]}>
          <HassuGrid sx={{ width: "100%" }} cols={[1, 1, 3]}>
            <TextInput
              label="Etunimi *"
              {...register(`jatkoPaatos1Vaihe.kuulutusYhteystiedot.yhteysTiedot.${index}.etunimi`)}
              error={(errors as any).jatkoPaatos1Vaihe?.kuulutusYhteystiedot?.[index]?.etunimi}
            />
            <TextInput
              label="Sukunimi *"
              {...register(`jatkoPaatos1Vaihe.kuulutusYhteystiedot.yhteysTiedot.${index}.sukunimi`)}
              error={(errors as any).jatkoPaatos1Vaihe?.kuulutusYhteystiedot?.[index]?.sukunimi}
            />
            <TextInput
              label="Organisaatio / kunta *"
              {...register(`jatkoPaatos1Vaihe.kuulutusYhteystiedot.yhteysTiedot.${index}.organisaatio`)}
              error={(errors as any).jatkoPaatos1Vaihe?.kuulutusYhteystiedot?.[index]?.organisaatio}
            />
            <TextInput
              label="Puhelinnumero *"
              {...register(`jatkoPaatos1Vaihe.kuulutusYhteystiedot.yhteysTiedot.${index}.puhelinnumero`)}
              error={(errors as any).jatkoPaatos1Vaihe?.kuulutusYhteystiedot?.[index]?.puhelinnumero}
              maxLength={maxPhoneLength}
            />
            <TextInput
              label="Sähköpostiosoite *"
              {...register(`jatkoPaatos1Vaihe.kuulutusYhteystiedot.yhteysTiedot.${index}.sahkoposti`)}
              error={(errors as any).jatkoPaatos1Vaihe?.kuulutusYhteystiedot?.[index]?.sahkoposti}
            />
          </HassuGrid>
