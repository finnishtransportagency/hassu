...
export function validateMuokkaustilaAllowsInput(
  ...
  if (muokkausTila === MuokkausTila.LUKU) {
    ...
    Object.keys(input).forEach((key) => {
      const allowedInputKeys: (keyof NahtavillaoloVaiheInput | keyof HyvaksymisPaatosVaiheInput)[] = [
        "aineistoNahtavilla",
        "alkuperainenPaatos",
      ];
      if (!allowedInputKeys.includes(key as keyof NahtavillaoloVaiheInput | keyof HyvaksymisPaatosVaiheInput)) {
        throw new IllegalArgumentError(`Et voi muokata arvoa ${key}, koska projekti on aineistomuokkaustilassa`);
      }
    });
  }
}
