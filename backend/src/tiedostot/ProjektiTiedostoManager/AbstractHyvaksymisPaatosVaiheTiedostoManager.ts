...
import { HyvaksymisPaatosVaihe, PaatosVaiheJulkaisu } from "../../database/model";

export abstract class AbstractPaatosVaiheTiedostoManager<T extends PaatosVaiheJulkaisu> extends VaiheTiedostoManager<
...
> {
  ...
  async deleteAineistotIfEpaaktiivinen(projektiStatus: Status): Promise<T[]> {
    if (!(isStatusGreaterOrEqualTo(projektiStatus, this.subsequentEpaaktiivinen) && this.julkaisut)) {
      return [];
    }
    const julkaisutSet = await this.julkaisut.reduce(async (modifiedJulkaisutPromise: Promise<Set<T>>, julkaisu) => {
