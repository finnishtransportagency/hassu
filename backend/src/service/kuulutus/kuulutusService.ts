import { KuulutusTyyppi, PDF } from "../../../../common/graphql/apiModel";
import { DBProjekti } from "../../database/model/projekti";
import log from "loglevel";

export class KuulutusService {
  async createPDF(projekti: DBProjekti, kuulutusTyyppi: KuulutusTyyppi): Promise<PDF> {
    if (kuulutusTyyppi !== KuulutusTyyppi.ALOITUSKUULUTUS) {
      throw new Error("Kuulutustyyppi ei ole vielä tuettu");
    }
    const kuulutus = projekti.aloitusKuulutus;
    if (!kuulutus) {
      throw new Error("Projektilla ei ole kuulutusta");
    }
    const doc = setupDocument();

    function paragraph(text?: string | null) {
      if (!text) {
        return undefined;
      }
      return doc.struct("P", [
        () => {
          doc.text(text).moveDown(1);
        },
      ]);
    }

    const logo = doc.struct(
      "Figure",
      {
        alt: "Elinkeino-, liikenne- ja ympäristökeskus",
      },
      [
        () => {
          const elyFileName = __dirname + "/files/ely.png";
          log.info(elyFileName);
          doc.image(elyFileName, undefined, undefined, { scale: 0.22 });
        },
      ]
    );

    doc.addStructure(
      doc.struct("Document", {}, [
        () => {
          doc.text("", 186).moveDown(2);
        },

        logo,
        doc.struct("H1", () => {
          doc.moveDown(1).font("ArialMTBold").fontSize(10).text("KUULUTUS SUUNNITTELUN ALOITTAMISESTA").moveDown(1);
        }),
        () => {
          const parts = [projekti.velho?.nimi];
          if (projekti.velho?.kunnat) {
            parts.push(...projekti.velho.kunnat);
          }
          // switch (kuulutus.projektiTyyppi) {
          //   case ProjektiTyyppi.TIE:
          parts.push("tiesuunnitelma");
          //     break;
          //   case ProjektiTyyppi.YLEINEN:
          //     parts.push("yleissuunnitelma");
          //     break;
          // }

          doc.text(parts.join(", "));
          doc.moveDown(1);
          doc.font("ArialMT");
        },

        paragraph(kuulutus.hankkeenKuvaus),

        paragraph(
          "Asianosaisten katsotaan saaneen tiedon suunnittelun käynnistymisestä ja tutkimusoikeudesta seitsemäntenä päivänä kuulutuksen julkaisusta (hallintolaki 62 a §)."
        ),
        paragraph(
          "Suunnitelmasta vastaavalla on oikeus tehdä kiinteistöillä suunnittelutyön edellyttämiä mittauksia, maaperätutkimuksia ja muita valmistelevia toimenpiteitä (laki liikennejärjestelmästä ja maanteistä LjMTL 16 §)."
        ),

        // Ohje:
        // Seuraava kappale koskee asianosaisten informointia suunnitteluun liittyvistä oikeuksista ja vaikutusmahdollisuuksista.

        paragraph(
          "Kiinteistön omistajilla ja muilla asianosaisilla sekä niillä, joiden asumiseen, työntekoon tai muihin oloihin suunnitelma saattaa vaikuttaa on oikeus olla tutkimuksissa saapuvilla ja lausua mielipiteensä asiassa (LjMTL 16 § ja 27 §)."
        ),
        paragraph(
          "Suunnittelun edetessä tullaan myöhemmin erikseen ilmoitettavalla tavalla varaamaan tilaisuus mielipiteen ilmaisemiseen suunnitelmasta ( LjMTL 27 § ja valtioneuvoston asetus maanteistä 3 §)."
        ),

        paragraph(
          "Valmistuttuaan suunnitelmat asetetaan yleisesti nähtäville, jolloin asianosaisilla on mahdollisuus tehdä kirjallinen muistutus suunnitelmasta (LjMTL 27 §)."
        ),

        // Ohje:
        // Ilmoitukseen / kuulutukseen lisätään maininta henkilötietojen käsittelystä ja ELYn/Väylän suunnittelua koskevan tietosuojaselosteen verkko-osoite.
        doc.struct("P", [
          () => {
            doc.text(
              "<Kirjoita tähän > käsittelee suunnitelman laatimiseen liittyen tarpeellisia henkilötietoja. Halutessasi tietää tarkemmin väyläsuunnittelun tietosuojakäytänteistä, tutustu verkkosivujen tietosuojaosioon osoitteessa ",
              { continued: true }
            );
          },
          doc.struct(
            "Link",
            { alt: "https://www.ely-keskus.fi/web/ely/tietosuoja-ja-henkilotietojen-kasittely" },
            () => {
              doc
                .fillColor("blue")
                .text("https://www.ely-keskus.fi/web/ely/tietosuoja-ja-henkilotietojen-kasittely", {
                  link: "https://www.ely-keskus.fi/web/ely/tietosuoja-ja-henkilotietojen-kasittely",
                  continued: true,
                  underline: true,
                })
                .fillColor("black");
            }
          ),
          () => {
            doc.text(".", { link: null, underline: false }).moveDown(1);
          },
        ]),

        paragraph(
          "Lisätietoja antavat < Kirjoita tähän > ELY-keskus / Väylävirasto <etunimi sukunimi>, puh. <nro>, <sähköposti> (ja < Kirjoita tähän > kunta/kaupunki <etunimi sukunimi>, puh. <nro>, <kirjoita tähän sähköposti >)"
        ),
        () => {
          doc.font("ArialMTBold");
          doc.text("<Kuuluttaja eli toimivaltainen viranomainen>").moveDown(1);
        },
      ])
    );

    return new Promise<PDF>((resolve) => {
      const buffers = Array<any>();
      doc.on("data", buffers.push.bind(buffers));
      doc.on("end", () => {
        const all = Buffer.concat(buffers);
        const sisalto = all.toString("base64");
        resolve({ __typename: "PDF", nimi: "aloituskuulutus.pdf", sisalto });
      });
      doc.end();
    });
  }
}

// const fs = require("fs");
const PDFDocument = require("pdfkit");

function setupDocument() {
  const doc = new PDFDocument({
    pdfVersion: "1.7ext3",
    size: "A4",
    tagged: true,
    info: { Title: "KUULUTUS SUUNNITTELUN ALOITTAMISESTA" },
    lang: "fi",
    displayTitle: true,
  });

  const SRGB_IEC61966_ICC_PROFILE_B64 =
    "";

  const xmp = `
  <?xpacket begin="\ufeff" id="W5M0MpCehiHzreSzNTczkc9d"?>
  <x:xmpmeta xmlns:x="adobe:ns:meta/">
      <rdf:RDF xmlns:rdf="http://www.w3.org/1999/02/22-rdf-syntax-ns#">
          <rdf:Description xmlns:pdfaid="http://www.aiim.org/pdfa/ns/id/" rdf:about="">
              <pdfaid:part>3</pdfaid:part>
              <pdfaid:conformance>B</pdfaid:conformance>
          </rdf:Description>
      </rdf:RDF>
  </x:xmpmeta>
  <?xpacket end="w"?>
`;

  // PDF/A standard requires embedded color profile.
  const colorProfile = Buffer.from(SRGB_IEC61966_ICC_PROFILE_B64, "base64");
  const refColorProfile = doc.ref({
    Length: colorProfile.length,
    N: 3,
  });
  refColorProfile.write(colorProfile);
  refColorProfile.end();

  // noinspection JSPrimitiveTypeWrapperUsage
  const refOutputIntent = doc.ref({
    Type: "OutputIntent",
    S: "GTS_PDFA1",
    // tslint:disable-next-line:no-construct
    Info: new String("sRGB IEC61966-2.1"),
    // tslint:disable-next-line:no-construct
    OutputConditionIdentifier: new String("sRGB IEC61966-2.1"),
    DestOutputProfile: refColorProfile,
  });
  refOutputIntent.end();

  // Metadata defines document type.
  const metadata = xmp.trim();
  const refMetadata = doc.ref({
    Length: metadata.length,
    Type: "Metadata",
    Subtype: "XML",
  });
  refMetadata.compress = false;
  refMetadata.write(Buffer.from(metadata, "utf-8"));
  refMetadata.end();

  // Add manually created objects to catalog.
  doc._root.data.OutputIntents = [refOutputIntent];
  doc._root.data.Metadata = refMetadata;

  // PDF/A standard requires fonts to be embedded.
  const arialFontFile = __dirname + "/files/arialmt.ttf";
  log.info(arialFontFile);
  doc.registerFont("ArialMT", arialFontFile);
  doc.registerFont("ArialMTBold", __dirname + "/files/ARIALBOLDMT.OTF");
  return doc;
}

export const kuulutusService = new KuulutusService();
