...
function pdfFileName({ tyyppi, kieli }: PdfEvent) {
  if (tyyppi === ProjektiTyyppi.TIE) {
    return `T415 Ilmoitus kiinteistonomistajat nahtaville asettaminen${kieli === Kieli.RUOTSI ? " sv" : ""}.pdf`;
  } else if (tyyppi === ProjektiTyyppi.RATA) {
    return `31R Ilmoitus kiinteistonomistajat nahtaville asettaminen${kieli === Kieli.RUOTSI ? " sv" : ""}.pdf`;
  } else {
    return `31YS Ilmoitus kiinteistonomistajat nahtaville asettaminen${kieli === Kieli.RUOTSI ? " sv" : ""}.pdf`;
  }
}
