export function formatKiinteistotunnusForDisplay(tunnus?: string | null) {
  return tunnus
    ? `${Number(tunnus.substring(0, 3))}-${Number(tunnus.substring(3, 6))}-${Number(tunnus.substring(6, 10))}-${Number(
        tunnus.substring(10)
      )}`
    : "";
}

export function formatKiinteistotunnusForDatabase(tunnus?: string | null): string | undefined {
  if (!tunnus) {
    return undefined;
  }
