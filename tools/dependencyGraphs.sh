#!/usr/bin/env bash
mkdir -p .report
FILTER="aws common error user model importContext adapter config util logger scheduledEvent.ts projektiUtil.ts projektirekisteri hakupalvelu aineistopalvelu PartiallyMandatory ProjektiPath.ts adaptToDB projektiSchemaUpdate.ts"
npx ts_dependency_graph --start ./backend/src/scheduler/sqsEventHandlerLambda.ts --graph_folder --filter $FILTER | dot -T png > .report/dependencygraph-sqsEventHandlerLambda.png
