#!/usr/bin/env bash
mkdir -p .report
FILTER="aws common error user model importContext adapter config util logger importAineistoEvent.ts projektiUtil.ts projektirekisteri hakupalvelu aineistopalvelu PartiallyMandatory ProjektiPath.ts adaptToDB"
npx ts_dependency_graph --start ./backend/src/aineisto/aineistoImporterLambda.ts --graph_folder --filter $FILTER | dot -T png > .report/dependencygraph-aineistoImporterLambda.png
